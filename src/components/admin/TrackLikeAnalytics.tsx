...
import { Heart, Users, Globe, Loader2, CalendarIcon } from "lucide-react";
...
const TrackLikeAnalytics = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [rowsRes, summaryRes] = await Promise.all([
        supabase.rpc("get_track_like_analytics", { start_date: start, end_date: end }),
        supabase.rpc("get_track_like_summary", { start_date: start, end_date: end }),
      ]);
      if (rowsRes.data) setRows(rowsRes.data as Row[]);
      if (summaryRes.data && summaryRes.data.length > 0) setSummary(summaryRes.data[0] as Summary);
      else setSummary(null);
    } catch (e) {
      console.error("Error fetching track like analytics:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const summaryCards = [
    { label: "Total Likes", value: summary?.total_likes ?? 0, icon: Heart },
    { label: "Unique Listeners", value: summary?.unique_listeners ?? 0, icon: Users },
    { label: "Top Country", value: summary?.top_country ?? "—", icon: Globe },
  ];

  return (
    <Card id="track-likes" className="p-8 bg-card/50 backdrop-blur-sm border-2 border-primary/40">
        ...
        <>
          ...
          {rows.length === 0 ? (
            <p className="text-muted-foreground font-['Rajdhani'] text-center py-4">
              No track heart clicks in this period.
            </p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="font-['Rajdhani'] text-center">Likes</TableHead>
                  <TableHead className="font-['Rajdhani'] text-center">Unique Listeners</TableHead>
                  <TableHead className="font-['Rajdhani'] text-right">Last Liked</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={`${r.title}-${r.artist}-${i}`}>
                    ...
                    <TableCell className="text-center font-['Rajdhani']">{r.total_likes}</TableCell>
                    ...
                    <TableCell className="text-right font-['Rajdhani'] text-muted-foreground">
                      {format(new Date(r.last_liked), "MMM d, yyyy HH:mm")}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
...
export default TrackLikeAnalytics;
